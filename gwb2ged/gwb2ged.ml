  ...
  if i = String.length s then fprintf oc "\n"
    ...
    then
      ...
      if !j = String.length s then fprintf oc "\n"
        ...
        display_note_aux oc tagn s
          (String.length (string_of_int (succ tagn) ^ " CONC ")) (!j + 1)
    else
      begin output_char oc c; display_note_aux oc tagn s (len + 1) (i + 1) end
...
let ged_pevent base oc per per_sel evt =
  let typ =
...
let ged_ind_ev_str base sel oc per per_sel =
