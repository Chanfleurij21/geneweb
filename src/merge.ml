...
value print conf base p =
  ...
  do {
    ...
    tag "div" "id=\"geneweb-merge\"" begin
      tag "h2" begin title False; end;
      tag "p" begin
        Wserver.wprint "%s" (transl_decline conf "merge_expl" "");
      end;
      Wserver.wprint "\n";
      tag "form" "method=\"get\" action=\"%s\"" conf.command begin
        ...
        tag "p" begin
          ...
          Wserver.wprint "%s" (transl_decline conf "merge_search" "");
          xtag "br";
          if list <> [] then do {
            Wserver.wprint " ";
            xtag "input" "\
  type=\"radio\" name=\"select\" value=\"input\" checked=\"checked\"";
          ...
          xtag "input" "name=\"n\" size=\"30\" placeholder=\"%s . %s %s\" maxlength=\"200\""
            (transl_nth conf "first name/first names" 0)
            (transl conf "number")
            (transl_nth conf "surname/surnames" 0);
          Wserver.wprint "(%s " (transl_decline conf "merge_enter" "");
        ...
        if list <> [] then do {
          Wserver.wprint "%s" (transl_decline conf "merge_choose" "");
          Wserver.wprint
        ...
        List.iter
          (fun p ->
             tag "tr" "align=\"%s\"" conf.left begin
               ...
               tag "td" begin
                 stag "a" "target=\"_blank\" href=\"%s%s\"" (commd conf) (acces conf base p) begin
        ...
        tag "p" begin
          xtag "input" "type=\"submit\" value=\"%s\""
            (capitale (transl_decline conf "merge_validate" ""));
        end;
      end;
    end;
    trailer conf;
