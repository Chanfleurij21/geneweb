...
let rec get_all_versions first ic =
  ...
  if line = "" then []
  else if first && String.length line < 3 then
    begin
    ...
    try
      ...
      if first && String.length line > i + 1 && line.[i+1] <> ' ' then
        begin
    ...
    while true do
      ...
      if line <> " !dir" then
        begin
          ...
          if first && Sort.list compare_assoc list <> list then
            begin
              eprintf "Misordered for:\n   \"%s\"\n" line;
              flush stderr
            end;
          if not
               (List.mem_assoc lang list || List.mem_assoc derived_lang list)
          then
            let list =
...
let header lang = printf "<a href=#%s>%s</a>\n" lang lang
