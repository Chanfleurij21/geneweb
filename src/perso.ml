...
let gen_interp_templ menu title templ_fname conf base p =
  ...
  let env =
    ...
    ["p", Vind p; "p_auth", Vbool (authorized_age conf base p);
     "count", Vcnt (ref 0); "count1", Vcnt (ref 0); "count2", Vcnt (ref 0);
     "list", Vslist (ref SortedList.empty); "desc_mark", Vdmark (ref [| |]);
     "lazy_print", Vlazyp (ref None); "sosa", Vsosa (ref []);
     "sosa_ref", Vsosa_ref sosa_ref_l; "t_sosa", Vt_sosa t_sosa;
